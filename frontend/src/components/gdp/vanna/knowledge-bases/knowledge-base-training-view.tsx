...
import { listTrainingEntries, listVannaSqlAssets } from "../shared/vanna-api"
import { SqlAssetPromoteDialog } from "./sql-asset-promote-dialog"
import type { VannaSqlAssetRecord, VannaTrainingEntryRecord } from "../shared/vanna-types"
...
export function KnowledgeBaseTrainingView({
  ...
  function handleCreateKnowledge() {
    if (entryType === "question_sql") {
      router.push(`/gdp/vanna/knowledge-bases/${kbId}/training/question-sql/new`)
      return
    }
    router.push(`/gdp/vanna/knowledge-bases/${kbId}/training/documentation/new`)
  }
                    ...
                    <div className="flex items-center gap-3">
                      {selectedPromotedAssets.length > 0 ? (
                        <Button
                          ...
                          onClick={() =>
                            router.push(
                              `/gdp/vanna/knowledge-bases/${kbId}/assets?entryId=${selectedItem.id}`
                            )
                          }
                        >
                          查看 SQL 资产
