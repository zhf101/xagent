...
export function HttpConfigDrawer({ open, onOpenChange, onSaved, assetId }: HttpConfigDrawerProps) {
  ...
  const [isEnvOptionsLoading, setIsEnvOptionsLoading] = useState(false)
  const [envOptionsError, setEnvOptionsError] = useState<string | null>(null)
  ...
  const selectedEnvOption = useMemo(
    () => envOptions.find(option => option.env_label === payload.execution_profile.sys_label),
    [envOptions, payload.execution_profile.sys_label]
  )
  const envLabelOptions: SelectOption[] = useMemo(
    () =>
      envOptions.map(option => ({
        value: option.env_label,
        label: `${option.env_label} · ${option.base_url}`,
        description:
          option.status === "active"
            ? (option.description || "使用该环境标签时会命中此基础地址")
  ...
  const loadEnvOptions = async (systemShort?: string, includeEnvLabel?: string) => {
    ...
    try {
      const params = new URLSearchParams()
      params.set("system_short", normalizedSystemShort)
      if (includeEnvLabel?.trim()) {
        params.set("include_env_label", includeEnvLabel.trim())
      }
      const response = await apiRequest(
        `${getApiUrl()}/api/system-registry/env-options?${params.toString()}`
      )
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    if (payload.execution_profile.url_mode !== "tag") {
      return
    }
    void loadEnvOptions(
      payload.resource.system_short,
      payload.execution_profile.sys_label
    )
  }, [
    open,
    payload.execution_profile.url_mode,
    payload.execution_profile.sys_label,
    payload.resource.system_short,
  ])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(assetId ? "接口资产已更新" : "接口资产已创建")
        onSaved()
        onOpenChange(false)
      } else {
            ...
            <div className="p-10 max-w-[850px] mx-auto min-h-full flex flex-col pb-32">
                              ...
                              {envOptionsError ? (
                                <>
                                  <Input
                                    ...
                                    onChange={e => updateProfile("sys_label", e.target.value.toUpperCase())}
                                    placeholder="环境标签，例如 PROD"
                                    className="h-11 rounded-xl"
                                  />
                                  <p className="text-xs text-amber-600">
                                    {envOptionsError}，当前回退为手动输入。
                                  </p>
                                </>
                              ) : (
                                <>
                                  <Select
                                    value={payload.execution_profile.sys_label || ""}
                                    onValueChange={value => updateProfile("sys_label", value)}
                                    options={envLabelOptions}
                                    placeholder={
                                      !payload.resource.system_short
                                        ? "请先选择所属系统"
                                        : isEnvOptionsLoading
                                    ...
                                    disabled={
                                      !payload.resource.system_short ||
                                      isEnvOptionsLoading ||
                                      envLabelOptions.length === 0
                                    }
                                  />
                                  {selectedEnvOption ? (
                                    <p className="text-xs text-muted-foreground">
                                      当前环境将命中: <span className="font-mono">{selectedEnvOption.base_url}</span>
                                    </p>
                                  ) : payload.resource.system_short ? (
                                    <p className="text-xs text-muted-foreground">
                                      当前系统尚未配置可用环境标签，请先到系统管理页补充环境地址。
                                    </p>
                                  ) : null}
                                </>
                              )}
                            </div>
                            <Input value={payload.execution_profile.url_suffix || ""} onChange={e => updateProfile("url_suffix", e.target.value)} placeholder="/api..." className="h-11 rounded-xl" />
                          </div>}
                      </div>
                    </div>
              ...
              <div className="mt-auto pt-16 flex justify-between items-center border-t border-dashed border-border/60">
                ...
                {activeStep !== STEPS[STEPS.length - 1].id ? 
                  ...
                  <Button onClick={handleSave} disabled={isSaving} className="rounded-full px-12 h-11 bg-primary shadow-xl shadow-primary/20">{isSaving ? "正在处理..." : "完成保存"}</Button>}
              </div>
            </div>
          </div>
        </div>
