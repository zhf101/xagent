...
import { GdpExecutionProfile, GdpHttpAssetPayload, GdpHttpResource } from "./gdp-types"
...
/**
 * 过滤 resource 字段，只保留后端协议定义的字段
 * 避免将 id, create_user_id, status 等数据库字段传给 normalize 接口
 */
function filterResourceFields(resource: GdpHttpResource): GdpHttpResource {
  return {
...
function buildBackendPayload(payload: GdpHttpAssetPayload): GdpHttpNormalizedPayload {
  return {
    ...payload,
    resource: filterResourceFields(payload.resource),
    execution_profile: {
