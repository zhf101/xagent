...
from xagent.config import get_external_skills_dirs
from xagent.skills.utils import _get_default_skill_dirs, create_skill_manager


class TestGetExternalSkillsDirs:
    """Tests for get_external_skills_dirs function in core/config.py"""

    def test_empty_env_var(self, monkeypatch):
        """Test empty environment variable returns empty list"""
        monkeypatch.delenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", raising=False)
        result = get_external_skills_dirs()
        assert result == []

    def test_single_valid_directory(self, monkeypatch):
        """Test single valid directory"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", tmpdir)
            result = get_external_skills_dirs()
            assert len(result) == 1
            assert result[0] == Path(tmpdir)

    def test_multiple_valid_directories(self, monkeypatch):
        """Test multiple valid directories"""
        with tempfile.TemporaryDirectory() as tmpdir1:
            with tempfile.TemporaryDirectory() as tmpdir2:
                monkeypatch.setenv(
                    "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", f"{tmpdir1},{tmpdir2}"
                )
                result = get_external_skills_dirs()
                assert len(result) == 2
    ...
    def test_nonexistent_directory_still_added(self, monkeypatch):
        """Test nonexistent directory is still added (may be created later)"""
        monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", "/nonexistent/path")
        result = get_external_skills_dirs()
        assert len(result) == 1
    ...
    def test_path_expansion_tilde(self, monkeypatch):
        ...
        monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", "~/test")
        result = get_external_skills_dirs()
        # Path is expanded and added even if it doesn't exist yet
        assert len(result) == 1
        assert result[0] == Path.home() / "test"

    def test_path_expansion_environment_variable(self, monkeypatch):
        """Test environment variable expansion"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv("TEST_SKILLS_DIR", tmpdir)
            monkeypatch.setenv(
                "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", "$TEST_SKILLS_DIR"
            )
            result = get_external_skills_dirs()
            assert len(result) == 1
            assert result[0] == Path(tmpdir)

    def test_mixed_expansions(self, monkeypatch):
        """Test mix of tilde and environment variable expansion"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv("TEST_SKILLS_DIR", tmpdir)
            monkeypatch.setenv(
                "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", "~/test,$TEST_SKILLS_DIR"
            )
            result = get_external_skills_dirs()
            assert len(result) == 2
            assert result[0] == Path.home() / "test"
            assert result[1] == Path(tmpdir)

    def test_whitespace_handling(self, monkeypatch):
        """Test whitespace handling in paths"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv(
                "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", f"  {tmpdir}  ,  {tmpdir}  "
            )
            result = get_external_skills_dirs()
            assert len(result) == 2

    def test_empty_components_are_skipped(self, monkeypatch):
    ...
    def test_url_paths_are_rejected(self, monkeypatch):
        """Test URL-like paths are rejected"""
        monkeypatch.setenv(
            "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS",
            "s3://bucket/skills,nfs://server/skills",
        )
        result = get_external_skills_dirs()
        assert result == []  # URLs should be rejected

    def test_mixed_valid_invalid(self, monkeypatch):
        """Test mix of valid and invalid directories"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv(
                "XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS",
                f"{tmpdir},/nonexistent,/another/nonexistent",
            )
            result = get_external_skills_dirs()
    ...
    def test_file_path_accepted(self, monkeypatch):
        ...
        try:
            monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", tmpfile_path)
            result = get_external_skills_dirs()
...
class TestGetDefaultSkillDirs:
    ...
    def test_returns_list_of_paths(self):
        """Test returns a list of Path objects"""
        result = _get_default_skill_dirs()
        assert isinstance(result, list)
        assert all(isinstance(p, Path) for p in result)

    def test_contains_builtin_dirs(self):
        """Test includes built-in skill directories"""
        result = _get_default_skill_dirs()
        # Should contain built-in skills directory
        assert any(p.name == "skills" for p in result)
...
class TestCreateSkillManager:
    ...
    def test_no_environment_variable(self, monkeypatch):
    ...
    def test_with_multiple_directories(self, monkeypatch):
        """Test with multiple directories in environment variable"""
        with tempfile.TemporaryDirectory() as tmpdir1:
            with tempfile.TemporaryDirectory() as tmpdir2:
                env_value = f"{tmpdir1},{tmpdir2}"
                monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", env_value)
                manager = create_skill_manager()
                assert manager is not None
    ...
    def test_with_invalid_environment_variable_falls_back_to_default(self, monkeypatch):
    ...
    def test_explicit_skills_roots_parameter(self, monkeypatch):
        """Test explicit skills_roots parameter is used and env var is appended"""
        with tempfile.TemporaryDirectory() as tmpdir:
            monkeypatch.setenv("XAGENT_EXTERNAL_SKILLS_LIBRARY_DIRS", "/other/path")
    ...
    def test_url_paths_in_environment_variable(self, monkeypatch):
    ...
    def test_path_expansion_in_environment_variable(self, monkeypatch):
...
class TestSkillManagerIntegration:
    ...
    def test_end_to_end_with_real_skills(self, monkeypatch):
    ...
    def test_default_behavior_without_config(self, monkeypatch):
