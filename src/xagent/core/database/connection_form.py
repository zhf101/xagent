...
这层模块的目标不是替代 adapter，而是把“人类更容易填写的连接表单”
和“系统实际需要的连接 URL”之间的转换逻辑统一收口。

设计原则：
- 前端只关心字段和交互，不自己硬编码不同数据库的 URL 拼接规则
- 后端保存与测试连接都共用这一套规则，避免“预览能拼、保存不能用”
- 全量覆盖当前支持的数据库类型；复杂场景允许高级模式直接提交原始 URL
"""
...
def _parse_extra_params(raw_value: Any) -> dict[str, str]:
    ...
    if isinstance(raw_value, str):
        ...
        # 允许前端 textarea 里按 key=value 一行一项填写。
        pairs: dict[str, str] = {}
    ...
    if normalized in {"mysql", "oceanbase", "tidb", "polardb", "goldendb"}:
        fields = [
            *_base_network_fields(profile),
            ...
            ConnectionFieldDefinition(
                ...
                description="每行一个 key=value，会拼接到连接字符串 query 参数中",
            ),
        ]
        ...
        fields = [
            ...
            ConnectionFieldDefinition(
                ...
                input_type="number",
                required=True,
                default_value=str(profile.get("default_port") or 6379),
            ),
            ConnectionFieldDefinition(
                key="username",
                label="用户名",
                placeholder="ACL 场景可填写",
                advanced=True,
            ),
            ConnectionFieldDefinition(
                key="password",
...
def build_connection_url(db_type: str, form: dict[str, Any]) -> str:
    ...
    if normalized in {"mysql", "oceanbase", "tidb", "polardb", "goldendb"}:
        return _build_network_url(
            ...
            port=_clean_int(form.get("port"), get_database_profile(normalized)["default_port"]),
    ...
    if normalized == "mongodb":
        if _clean_string(form.get("auth_database")):
            extra["authSource"] = _clean_string(form.get("auth_database"))
        return _build_network_url(
            scheme=normalized,
            host=_clean_string(form.get("host")),
            port=_clean_int(form.get("port"), get_database_profile(normalized)["default_port"]),
    ...
    if normalized == "redis":
        database = _clean_string(form.get("redis_db")) or "0"
        return _build_network_url(
            scheme=normalized,
            host=_clean_string(form.get("host")),
            port=_clean_int(form.get("port"), get_database_profile(normalized)["default_port"]),
            ...
            path=database,
            query=extra,
            password_only_auth=not _clean_string(form.get("username")),
        )
...
def parse_connection_url(db_type: str, url: str) -> dict[str, Any]:
    ...
    if normalized == "sqlite":
        ...
        extra = {key: value for key, value in query.items()}
    else:
        form = {
            "host": parsed.hostname or "",
            "port": str(parsed.port or get_database_profile(normalized).get("default_port") or ""),
            "username": unquote(parsed.username or ""),
            "password": unquote(parsed.password or ""),
        }
        extra = dict(query)

        if normalized in {"mysql", "oceanbase", "tidb", "polardb", "goldendb", "postgresql", "kingbase", "gaussdb", "vastbase", "highgo", "sqlserver", "mongodb", "clickhouse", "dm"}:
            form["database"] = _extract_path_value(url)
        if normalized == "redis":
            form["redis_db"] = _extract_path_value(url) or "0"
    ...
    if normalized == "oracle":
        ...
        form["auth_database"] = extra.pop("authSource", "")
    elif normalized == "clickhouse":
