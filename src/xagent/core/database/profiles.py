...
DATABASE_PROFILES: tuple[DatabaseProfile, ...] = (
    ...
    DatabaseProfile(
        ...
        notes=("走 PostgreSQL 兼容族 adapter。",),
    ),
    DatabaseProfile(
        db_type="redis",
        display_name="Redis",
        default_port=6379,
        category="NoSQL",
        protocol="redis-json-command",
        support_level="full",
        aliases=(),
        driver_packages=("redis",),
        connection_example="redis://:password@host:6379/0",
        notes=("query 需传 JSON 命令协议，而不是 SQL。",),
    ),
    DatabaseProfile(
        db_type="oracle",
        display_name="Oracle",
    ...
    DatabaseProfile(
        ...
        notes=("默认走 pymssql；如需 ODBC 可在 URL query 中附加 driver。",),
    ),
    DatabaseProfile(
        db_type="mongodb",
        ...
        default_port=27017,
        category="NoSQL",
        protocol="mongodb-json-command",
        support_level="full",
        aliases=("mongo",),
        driver_packages=("pymongo",),
        connection_example="mongodb://user:password@host:27017/database",
        notes=("query 需传 JSON 协议，支持 find/aggregate/update_many 等操作。",),
    ),
    DatabaseProfile(
        db_type="sqlite",
        display_name="SQLite",
    ...
    DatabaseProfile(
        ...
        notes=("当前通过 ODBC 执行；需要本机安装达梦 ODBC 驱动。",),
    ),
    DatabaseProfile(
        db_type="kingbase",
        display_name="人大金仓",
