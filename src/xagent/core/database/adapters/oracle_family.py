...
class OracleFamilyAdapter(SqlAlchemySyncAdapter):
    ...
    async def connect(self) -> None:
        """达梦走 ODBC 生命周期，不应落到 SQLAlchemy engine 预热。"""

        if self.config.db_type != "dm":
            await super().connect()
            return

        # 这里显式做一次轻量连接校验，确保 `/connection/test`、直连 SQL、
        # 以及任何先 `connect()` 再 `execute_query()` 的调用链都能在统一入口
        # 暴露驱动/DSN 配置错误，而不是在 build_sqlalchemy_url() 提前崩掉。
        with self._connect_dm():
            return

    async def disconnect(self) -> None:
        """达梦当前按次建连执行，没有需要持久释放的 SQLAlchemy engine。"""
