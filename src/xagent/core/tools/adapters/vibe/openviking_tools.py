...
提供 Agent 访问 OpenViking 长期记忆、索引资源和层次化上下文的能力。
这些工具作为 Resource Plane（资源层）的一部分，不参与业务决策，
只负责提供原始的上下文检索和关系图谱能力。
"""
...
from .....providers.openviking import get_openviking_service
...
class OpenVikingSearchTool(AbstractBaseTool):
    ...
    用于从 OpenViking 检索长期记忆、索引资源或层次化上下文。
    作为 Resource Plane 的一部分，只负责返回原始检索结果。
    """
...
class OpenVikingReadContextTool(AbstractBaseTool):
    ...
    支持 abstract / overview / read 三个层级，
    优先使用 abstract 或 overview 控制 token 消耗。
    """
...
class OpenVikingGrepTool(AbstractBaseTool):
    """
    OpenViking 内容级 grep 工具。
    
    当已知关键词或代码片段，需要精确匹配时使用。
    """
...
class OpenVikingRelationsTool(AbstractBaseTool):
    ...
    用于检查已知 URI 的关系链接。
    不要用作搜索的替代品；如果还不知道 URI，应先使用 openviking_search。
    """
...
async def create_openviking_tools(config: "BaseToolConfig") -> list[Any]:
    """
    按配置创建 OpenViking 工具集。
    
    只有在 OpenViking 搜索功能启用且 user_id 可用时才创建工具。
    """
