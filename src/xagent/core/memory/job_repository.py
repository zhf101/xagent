...
`JobManager` 负责“决定要入什么任务”，
`JobRepository` 负责“具体怎么写库、怎么领任务、怎么更新状态”。
"""
...
class MemoryJobRepository:
    ...
    def find_duplicate_open_job(
        ...
        """按 dedupe_key 查找仍然处于可执行/执行中的重复任务。"""
    ...
    def claim_next_job(
        ...
        *,
        ...
        这里会先筛选 claimable 候选，再用 update 抢占，
        避免多个 worker 同时拿到同一条 job。
        """
        now = now or datetime.utcnow()
