...
这个文件解决两个历史问题：
1. 老项目只有 `category`，语义比较松散，不适合继续扩展。
2. 新版需要更稳定的结构化字段，区分 memory_type / memory_subtype / scope。

因此这里既定义“新字段”，也保留“旧 category 到新结构”的兼容映射。
"""
...
def resolve_memory_type(
    ...
    """把旧数据里的 category 归一化成新版 memory_type。"""
    if memory_type:
        return memory_type
    if category:
...
def resolve_memory_subtype(
    ...
    *,
    ...
    """在显式 subtype 缺失时，根据旧 category 或 metadata 推断 subtype。"""
    if memory_subtype:
...
def default_category_for_type(memory_type: Optional[str]) -> str:
    """为新版 memory_type 反推一个兼容旧接口的默认 category。"""
    if not memory_type:
        return "general"
...
def matches_memory_filter(
    *,
    ...
    `InMemoryMemoryStore` 和 `LanceDBMemoryStore` 都会复用这套规则，
    这样内存实现和向量库实现的过滤语义保持一致。
    """
    metadata = metadata or {}
