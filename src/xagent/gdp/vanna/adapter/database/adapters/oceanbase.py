...
class OceanBaseAdapter(SqlAlchemySyncAdapter):
    family = "mysql"
    supported_types = ("oceanbase",)
    system_databases = (
        "information_schema",
        "mysql",
        "oceanbase",
    ...
    def _build_engine_connect_args(self) -> dict[str, Any]:
        return {
            "charset": str(
                (self.config.extra or {}).get("charset", "utf8mb4")
            ),
            "connect_timeout": self._get_extra_int("connect_timeout", 10),
            "read_timeout": self._get_extra_int("read_timeout", 60),
            "write_timeout": self._get_extra_int("write_timeout", 60),
        }
    ...
    def _assemble_schema(
        ...
        *,
        ...
        return {
            ...
            "harvestStrategy": "information_schema_current_database",
            "systemDatabasesKnown": list(self.system_databases),
            "isSystemDatabase": database_name.lower() in self.system_databases,
            "version": version,
            "tables": tables,
