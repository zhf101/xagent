...
class SqlAssetRetriever:
    """基于 LanceDB ANN 的 SQL 资产粗召回器。"""
    ...
    def recall(
        ...
        - embedding 模型不可用
        - 查询向量生成失败
        Fallback 行为：返回全量 active SQL 资产列表，每条补充 _distance=0.5。
        """
        ...
        if query_vec is None or table is None:
            logger.info(
                "SqlAssetRetriever fallback 到全量加载",
    ...
    def _fallback(self, system_short: str | None) -> list[dict[str, Any]]:
        assets = self._repository.list_active_sql_assets(system_short=system_short)
